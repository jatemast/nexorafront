...
async function handleSubmit() {
  ...
  try {
    ...
    const created = await evaluationsStore.create(payload)

    toast.add({
      severity: 'success',
      summary: 'Evaluation Created',
      detail: 'The evaluation has been created successfully.',
      life: 4000,
    })
...
<template>
  <div class="evaluations-create-page space-y-6">
    <!-- =================================================================== -->
    <!-- PAGE HEADER                                                         -->
    <!-- =================================================================== -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-surface-500 dark:text-surface-500 mt-1">
          Configure a new evaluation for learners
        </p>
      </div>
      <Button
    ...
    <template v-else>
      ...
      <div class="card p-6 sm:p-8 space-y-6">
        ...
        <p class="text-xs text-surface-400 dark:text-surface-500 -mt-4">
          Fill in the information for the new evaluation
        </p>

        <!-- Title -->
        <div>
          <label
            for="create-eval-title"
            class="block text-sm font-medium text-surface-700 dark:text-surface-600 mb-1.5"
          >
            Title <span class="text-error-500">*</span>
          </label>
          <InputText
            id="create-eval-title"
            v-model="title"
        ...
        <div>
          <label
            for="create-eval-description"
            class="block text-sm font-medium text-surface-700 dark:text-surface-600 mb-1.5"
          >
            Description <span class="text-error-500">*</span>
          </label>
          <Textarea
            id="create-eval-description"
            v-model="description"
        ...
        <div>
          <label
            for="create-eval-status"
            class="block text-sm font-medium text-surface-700 dark:text-surface-600 mb-1.5"
          >
            Status <span class="text-error-500">*</span>
          </label>
          <Select
            id="create-eval-status"
            v-model="status"
        ...
        <div class="pt-4 border-t border-surface-100 dark:border-surface-200">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <!-- Questions Count (read-only, updated dynamically) -->
            <div>
              <label
                for="create-eval-qcount"
                class="block text-sm font-medium text-surface-700 dark:text-surface-600 mb-1.5"
              >
                Questions Count
              </label>
              <InputNumber
                id="create-eval-qcount"
            ...
            <div>
              <label
                for="create-eval-timelimit"
                class="block text-sm font-medium text-surface-700 dark:text-surface-600 mb-1.5"
              >
                Time Limit (minutes) <span class="text-error-500">*</span>
              </label>
              <InputNumber
                id="create-eval-timelimit"
                v-model="timeLimit"
                :min="1"
                :max="480"
                suffix=" min"
            ...
            <div>
              <label
                for="create-eval-passing"
                class="block text-sm font-medium text-surface-700 dark:text-surface-600 mb-1.5"
              >
                Passing Score (%) <span class="text-error-500">*</span>
              </label>
              <InputNumber
                id="create-eval-passing"
            ...
            <div>
              <label
                for="create-eval-attempts"
                class="block text-sm font-medium text-surface-700 dark:text-surface-600 mb-1.5"
              >
                Max Attempts <span class="text-error-500">*</span>
              </label>
              <InputNumber
                id="create-eval-attempts"
                v-model="maxAttempts"
                :min="1"
                :max="50"
                class="w-full"
      ...
      <div class="flex items-center justify-end gap-3 pt-2">
        ...
        <Button
          icon="pi pi-check"
          :label="saving ? 'Creating...' : 'Create Evaluation'"
          severity="primary"
          :loading="saving"
          :disabled="saving"
          @click="handleSubmit"

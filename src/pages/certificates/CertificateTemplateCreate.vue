...
async function handleSubmit() {
  ...
  try {
    const payload = {
      ...
      logo: logoPreview.value || null,
      background_image: backgroundPreview.value || null,
      signature: signaturePreview.value || null,
      dynamic_fields: enabledDynamicFields.value.map((f) => f.key),
    }

    await certificatesStore.createTemplate(payload)

    toast.add({
      severity: 'success',
      summary: 'Template Created',
      detail: 'The certificate template has been created successfully.',
      life: 4000,
    })
...
// ---------------------------------------------------------------------------
onMounted(() => {
  // No initial data fetch needed for create form
  pageLoading.value = false
})
</script>

<template>
  <div class="certificate-template-create-page space-y-6">
    <!-- =================================================================== -->
    <!-- PAGE HEADER                                                         -->
    <!-- =================================================================== -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-surface-500 dark:text-surface-500 mt-1">
          Design a new certificate template with custom branding
        </p>
      </div>
      <Button

...
// Step 3: Lesson state is derived from modules
...
async function handleSubmit() {
  ...
  try {
    ...
    const created = await coursesStore.create(formData)

    toast.add({
      severity: 'success',
      summary: 'Course Created',
      detail: `"${created.title || title.value}" has been created successfully.`,
      life: 4000,
    })
...
<template>
  <div class="courses-create-page space-y-6">
    <!-- =================================================================== -->
    <!-- PAGE HEADER                                                         -->
    <!-- =================================================================== -->
    ...
    <div v-if="activeStep === 0" class="card p-6 sm:p-8">
      ...
      <p class="text-xs text-surface-400 dark:text-surface-500 mb-6">
        Provide the essential details for this course
      </p>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 space-y-3">
                ...
                <div>
                  <label
                    :for="'les-title-' + modIndex + '-' + lesIndex"
                    class="block text-xs font-medium text-surface-600 dark:text-surface-500 mb-1"
                  >
                    Lesson Title <span class="text-error-500">*</span>
                  </label>
                  <InputText
                    :id="'les-title-' + modIndex + '-' + lesIndex"
                ...
                <div>
                  <label
                    :for="'les-content-' + modIndex + '-' + lesIndex"
                    class="block text-xs font-medium text-surface-600 dark:text-surface-500 mb-1"
                  >
                    Content <span class="text-surface-400 font-normal">(optional)</span>
                  </label>
                  <Textarea
                    :id="'les-content-' + modIndex + '-' + lesIndex"
    ...
    <div class="flex items-center justify-between pt-2">
      ...
      <div class="flex items-center gap-3">
        ...
        <Button
          v-else
          icon="pi pi-check"
          :label="saving ? 'Creating...' : 'Create Course'"
          severity="primary"
          :loading="saving"
          :disabled="saving"
          @click="handleSubmit"
